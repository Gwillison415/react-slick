...
describe("Hackermaps", () => {
  ...
  beforeEach(() => {
    ...
    getByLabelText = app.getByLabelText;
    fireEvent.click(getByLabelText("open drawer"));
    fireEvent.click(getByTestId("hackermaps"));
  });
  ...
  const getLabelWithin = (dataTestId, labelText) => {
    const parent = getByTestId(dataTestId);
    return within(parent).getByLabelText(labelText);
  };
  ...
  it("Should render all the locations in the list", () => {
    ...
    expect(
      getWithin(IDMAPS.LOCATION, IDMAPS.LOCATIONS[0]).innerHTML.trim()
    ).toEqual(locations[0].trim());
    expect(
      getWithin(IDMAPS.LOCATION, IDMAPS.LOCATIONS[1]).innerHTML.trim()
    ).toEqual(locations[1].trim());
    expect(
      getWithin(IDMAPS.LOCATION, IDMAPS.LOCATIONS[2]).innerHTML.trim()
    ).toEqual(locations[2].trim());
    expect(
      getWithin(IDMAPS.LOCATION, IDMAPS.LOCATIONS[3]).innerHTML.trim()
    ).toEqual(locations[3].trim());
    expect(
      getWithin(IDMAPS.LOCATION, IDMAPS.LOCATIONS[4]).innerHTML.trim()
    ).toEqual(locations[4].trim());
  });
